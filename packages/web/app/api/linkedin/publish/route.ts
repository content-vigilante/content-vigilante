...
export async function POST(req: Request) {
  ...
  if (!token) {
    return NextResponse.json({ error: 'Not connected. Visit /api/linkedin/connect first.' }, { status: 401 });
  }
  ...
  if (!res.ok) {
    ...
    return NextResponse.json({ error: `LinkedIn publish failed: ${err.slice(0, 400)}` }, { status: 502 });
  }
